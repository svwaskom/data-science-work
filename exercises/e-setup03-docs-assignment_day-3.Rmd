...
```
<!-- answer-begin -->
The vignettes available with `dplyr` are:
...
look up help by:
 ...
 - using `?function_name` to find the help doc
 - just calling `function_name` to get  the code defining the function
...
```
<!-- answer-begin -->
the functions come from the following packages:

 - `cut_number` from `ggplot2`
 - `parse_number` from `readr`
