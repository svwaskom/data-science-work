...
```
**Observations:**
- `suv` seems to be the most frequent class
- the `subcompact` `class1 has the broadest distribution of cty values
- `q1-vis1` helped with the 1st question, where `q1-vis2` helped with the second
...
**Observations**
- with `binwidth = 0.01`, there seem to be highs at `0.3, 0.4, 0.5, 0.7, 0.9, and 1.0` values of `carat`.  counts decrease after each of those until the next one
- increasing `binwidth` to `0.001` shows a high at `0.9` that wasn't visible before
- decreasing `bindwidth` to `0.1` clearly shows that the high at `0.3` is higher than the rest
