...
cursor.
<!-- answer-begin -->
ggplot2, purrr, tibble, dpylr, tidyr, stringr, readr, and forcats
<!-- answer-end -->

__q3__ What are the main differences between `install.packages()` and `library()`? How often do you need to call each one?

<!-- answer-begin -->
`install.packages()` literally installs the package(s) on your machine, and only needs to be called once.  `library()` loads the contents of the package(s) into your session, and needs to be called at the beginning of every session (so, each time you close RStudio and reopen it, or log out & back in). 

Note from Zach's solution: Packages are frequently updated. You can check for updates and update
your packages by using the Update button in the Packages tab of RStudio
(lower right pane).
<!-- answer-end -->
