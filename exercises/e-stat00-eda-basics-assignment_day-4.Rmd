...
**Observations**:
- Nope!  In fact, there are more `Ideal` cut diamonds than any other cut.
...
**Observations**:
- This is not a helpful graph.  Should use a boxplot when one variable is categorical & the other is continuous.
...
**Observations**:
- Nope again!  `Ideal` cut diamonds are the *least* pricey.
...
**Observations**:
- Yes; the `Ideal` cut diamonds tend to have lower `carat` values, which supports the idea that they're sacrificing the `carat` value to get to that `Ideal` cut
- lower-quality `cut`s tend to have higher `carat` values, which I guess makes sense, since they're literally cutting away `carat`s to get to a different / better `cut`.
- apparently `carat` does more to increase price than `cut`, which is how the `Ideal` `cut` diamonds are cheaper than the lower-quality `cut` diamonds that tend to have higher `carat` values.
